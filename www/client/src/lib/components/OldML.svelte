<script lang="ts">
	...
	// ===============================================================
	let { chat = $bindable(), submitFn }: { chat: UsedChat; submitFn: (event: SubmitEvent) => void } =
		$props();
	let {
		messages,
		latestMessages,
		lastSequence: unreadCount,
		receivedUnreadCount,
		receivedNewCount,
		lastModified
	} = $derived(chat);
	...
	const oldUnreadCount = chat.lastSequence - receivedNewCount;
	...
	// ===============================================================
	export async function recalculateIndexes(direction: -1 | 0 | 1): Promise<void> {
	...
	async function handleBottomIntersection(): Promise<void> {
		...
		if (currentUnreadDelta < unreadCount && unreadCount) {
			...
			chat.lastSequence = newUnreadCount;
