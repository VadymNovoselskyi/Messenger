...
import { getChats, getExtraMessages, getExtraNewMessages, readUpdate, sendMessage, findUser, createUser, createChat, } from "./api.mjs";
...
function sendResponse(ws, api, id, status, payload) {
    return ws.send(JSON.stringify({ api, id, status, payload }));
}
;
const onlineUsers = {}; // Tracks active connections by user ID.
...
wss.on("connection", ws => {
    ...
    ws.on("message", async (message) => {
        ...
        const { api, id, payload } = parsedMessage;
    ...
    async function handleAuthenticatedCall(api, id, payload, userId) {
        try {
            switch (api) {
                case "get_chats": {
                ...
                case "read_update": {
                    ...
                    if (onlineUsers[receivingUID]) {
                        onlineUsers[receivingUID].send(JSON.stringify({
                            api,
                            status: "success",
