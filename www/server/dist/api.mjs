...
export async function getChats(userId) {
    try {
        ...
        userChats = await Promise.all(userChats.map(async (chat) => {
            const lastSeen = chat.users.find((user) => user._id.toString() === userId).lastSeen;
...
export async function getExtraMessages(chatId, currentIndex) {
    try {
...
export async function getExtraNewMessages(chatId, unreadCount) {
    try {
...
export async function readUpdate(userId, chatId, messageId) {
    try {
        const message = await messages.findOne({ _id: new ObjectId(messageId) });
        if (!message)
            throw new Error(`Didnt find message ${messageId}`);
        const { sendTime } = message;
        await chats.updateOne({ _id: new ObjectId(chatId) }, { $set: { "users.$[user].lastSeen": sendTime } }, { arrayFilters: [{ "user._id": new ObjectId(userId) }] });
        const chat = await chats.findOne({ _id: new ObjectId(chatId) });
        if (!chat)
            throw new Error(`Couldnt find chat for chatId ${chatId}`);
...
export async function sendMessage(userId, chatId, message) {
    try {
        ...
        const { modifiedCount } = await chats.updateOne({ _id: new ObjectId(chatId) }, {
        ...
        const chat = await chats.findOne({ _id: new ObjectId(chatId) });
        if (!chat)
            throw new Error(`Couldnt find chat with chatId ${chatId}`);
        const receivingUID = chat.users.find((user) => user._id.toString() !== userId)._id;
